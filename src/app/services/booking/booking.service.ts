...
export class BookingService {
  ...
  getAlertBookingSuccessMessage(bookedDatesArray: Date[]): string {
    return `You have booked this place from ${bookedDatesArray[0].getDate()}.${bookedDatesArray[0].getMonth() +
      1}.${bookedDatesArray[0].getFullYear()}
  to ${bookedDatesArray[
    bookedDatesArray.length - 1
  ].getDate()}.${bookedDatesArray[bookedDatesArray.length - 1].getMonth() +
      1}.${bookedDatesArray[bookedDatesArray.length - 1].getFullYear()}`;
  }
  ...
  setBookedDates(
    ...
    return this.db
      ...
      .doc<PropertyData>(propertyId)
      .valueChanges()
      .pipe(first())
      .pipe(
        map((propertyData: PropertyData) => {
          const datesFromBD: Date[] = propertyData.bookedDates
            ? propertyData.bookedDates.map(date => (date = date.toDate()))
            : [];
          let updatedDates: Date[];

          if (
            !this.checkIfTwoDateArraysHaveCommonElement(
              bookedDatesArray,
              datesFromBD
            )
          ) {
  ...
  saveBookingDetails(propertyId: string, bookingData: BookingData): void {
    ...
    this.db
      .collection("bookings")
      .doc(propertyId)
      .set({ [timeOfBooking]: bookingData }, { merge: true });
  }

  checkIfTwoDateArraysHaveCommonElement(
    dateArr1: Date[],
    dateArr2: Date[]
  ): boolean {
    let result = false;
