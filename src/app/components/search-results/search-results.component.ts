...
export class SearchResultsComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    this.routeParamsSubscription = this.route.queryParamMap.subscribe(
      (paramMap: ParamMap) => {
        const searchInputParams = new SearchInputPropertyData(
          ...
          (paramMap
            .getAll("priceRange")
            .map((num: string): number => +num) as PriceRange) || null,
  ...
  deleteProperty(propertyId: string): void {
    ...
    modalRef.componentInstance.isDeleted.subscribe((isDeleted: boolean) => {
      if (isDeleted) {
        this.properties = this.properties.filter((property: PropertyData) => property.timestamp.toString() !== propertyId);
      }
      modalRef.close();
    });
  }
